    ...
    public List<List<Boolean>> getWorld(@RequestBody List<List<Boolean>> currentWorld){
        ...
        for(int row = 0; row < currentWorld.size(); row++) {
            ...
            for(int column = 0; column < currentWorld.get(row).size(); column++) {
                newRow.add(new CelDeterminator().determineNextStatus(Cel.of(currentWorld.get(row).get(column)), NeighbourHoodFactory.create(currentWorld, new Location(row, column)).getAmountOfLivingNeigbours()).getCelValue());
            }
            newWorld.add(newRow);
        }
        return newWorld;
